...
% If considered, it's also a convex optimization problem, can also use Lagrangian + water-filling, whether to consider can be used as two curve results
...
for idxOfSat = 1 : NumOfServSatCur
 ...
 for idxOfSche = 1 : interface.NumOfBeamFoot(idxOfSche)
 for idxOfFoot = 1 : interface.tmpSat(idxOfSat).NumOfBeamFoot(idxOfSche)
 % Get number of users in current beam position
 ...
 % When facing maximization assignment problem, can find the maximum element m from coefficient matrix C, then subtract all elements from m to get coefficient matrix B, then the minimization assignment problem with B as coefficient matrix has the same optimal solution as the original maximization assignment problem with C as coefficient matrix.
...
% res = assignment(i);
 ...
 % Calculateall
 PosOfBeam = zeros(NumOfLightBeamfoot, 2); % beam positioncentercoordinate 
 ...
 % Calculateuser matrix
 n = numOfUserInFoot;% matrixdimension
 HungMat = zeros(n, n);
 hMat = zeros(n,n);
 for i = 1 : n % user
 curUser = orderOfUsrs(i);
 ...
 %% Allocate power: Lagrangian + water-filling
 % The result should be a ratio of each user's power to total power within beam position
 h = zeros(1, n);
 for i = 1 : n % User
 curUser = orderOfUsrs(i);
...
% thisBand current frequency band
 ...
 Pt = lightPower(footIdx) / length(Usrs); % Power allocated to each user
 ...
 index = find(lightFoot == curFoot);% Find the allocation order
 if index > 1 %
 for bfIdx = 1 : index - 1 % Calculate interference with previously allocated beam positions
 ...
 % First allocation, only need SNR, so no need to calculate interference
