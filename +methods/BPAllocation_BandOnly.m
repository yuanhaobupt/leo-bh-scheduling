...
for idxOfSat = 1 : NumOfServSatCur
 ...
 % Iterate to solve, refer to the power allocation method in the paper "Adaptive Beam Power Allocation Algorithm in Dynamic Beam Hopping Strategy", adaptive allocation
 % But as B increases, co-channel interference also increases emmm
 while true
 % Calculate average C
 a = zeros(1,n);
 for ii = 1 : n % User
...
% a(ii) = (allocBand(ii, 2) - allocBand(ii, 1)) * log2(1 + avgPower * (3e8/curFc /(4 * pi * d(ii)))^2 * Gsat * Gusr / N_noise_usr); 
 ...
 for ii = 1 : n % assignment index
 cur = assignment(ii);% Allocate in the order of assignment
...
% temp_a(ii) = (tempAllocBand(ii, 2) - tempAllocBand(ii, 1)) * log2(1 + avgPower * (3e8/curFc /(4 * pi * d(ii)))^2 * Gsat * Gusr / N_noise_usr); 
 ...
 % Compare two results, if not better, break, otherwise update next time
...
% h = loss;
 ...
 allo_set = 1:K; % Initialize the set of user indices to water-fill
